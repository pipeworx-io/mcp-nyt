 ...
 *
 * LICENCE (fleet #1980). NYT's own FAQ defines "has a paid tier" as
 * commercial, and the Developer Terms of Use §1(e)(v) forbid sharing access
 * "whether for direct commercial or monetary gain or otherwise" without
 * written permission from NYT. Bruce ruled 2026-09-15 (decision on fleet
 * #1980, same posture already accepted for last-fm/watchmode/guardian/acled
 * on 2026-08-18): zero-rate and attribute rather than ask NYT Licensing for
 * written permission. Two mechanisms, both required:
 *   - `zeroRated: true` on the gateway's pack entry (workers/gateway/src/
 *     index.ts) — 0 credits on every tier, no pricing-v2 bracket advance;
 *   - the `attribution` field on every response, below — travels into
 *     ask_pipeworx answers because the answer path passes a pack's
 *     structuredContent through verbatim.
 * Note the honest limit of this, so nobody later mistakes it for compliance:
 * NYT's FAQ still calls a paid-tier app commercial, so zero-rating is good
 * faith, not cure. Do not remove either mechanism, and do not re-introduce
 * a non-zero pack meter.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'NYT');
}

const BASE_URL = 'https://api.nytimes.com';

// Required by the Developer Terms of Use — NYT must be clearly identified as
// the source. Attached centrally in callTool (below), not per handler, so a
// new tool or an error-shaped response cannot ship unattributed by omission.
const NYT_ATTRIBUTION =
  'Data provided by The New York Times (developer.nytimes.com). Non-commercial use only under the '
  + 'NYT Developer Terms of Use; Pipeworx charges nothing for this data.';
...
async function nytGet(
  ...
  const res = await pwFetch(`${BASE_URL}${path}?${params.toString()}`);
  if (!res.ok) {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (result && typeof result === 'object' && !Array.isArray(result)) {
    return { attribution: NYT_ATTRIBUTION, ...(result as Record<string, unknown>) };
  }
  return { attribution: NYT_ATTRIBUTION, data: result };
}
...
// Do not meter this pack — zeroRated: true on the gateway pack entry is what
// actually keeps the call at 0 credits and out of pricing-v2 bracket counting;
// a non-zero meter here would be inconsistent with that and easy to miss.
